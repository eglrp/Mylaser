...
void ParamWindow::initActionsAndSlots()
{
    ...
    connect(ui->s_laserRatio, SIGNAL(valueChanged(int)), this, SLOT(on_laserRatioValueChanged()));
    connect(ui->s_laserFrequency, SIGNAL(valueChanged(int)), this, SLOT(on_laserFrequencyValueChanged()));
}
...
// 更新激光示意图表
/// @param ratio: 占空比
/// @param: frequency:激光频率
void ParamWindow::updateLaserChart(float ratio, int frequency)
{
    if(ratio<0 || frequency<=0) return;
    int tHigh = 1000000.f/(float)frequency*ratio;    //单位：us
    int tLow = 1000000.f/(float)frequency-tHigh;
    ui->lb_highPulseTime->setText(QString::number(tHigh)+"us");
    ui->lb_lowPulseTime->setText(QString::number(tLow)+"us");
}
...
void ParamWindow::on_laserRatioValueChanged()
{
    ui->lb_laserRatio->setText(QString::number(ui->s_laserRatio->value()));
    updateLaserChart((float)(ui->s_laserRatio->value())/100.f, ui->s_laserFrequency->value());
}

void ParamWindow::on_laserFrequencyValueChanged()
{
    ui->lb_laserFrequency->setText(QString::number(ui->s_laserFrequency->value()));
    updateLaserChart((float)(ui->s_laserRatio->value())/100.f, ui->s_laserFrequency->value());
}
